...
struct rlbox_noopstaticindirect_sandbox_thread_data
{
  rlbox_noopstaticindirect_sandbox* sandbox;
  uint32_t last_callback_invoked;
};

__attribute__((noinline))
static void* hide_val_id(void* val) { return val; }

#ifdef RLBOX_EMBEDDER_PROVIDES_TLS_STATIC_VARIABLES

rlbox_noopstaticindirect_sandbox_thread_data* get_rlbox_noopstaticindirect_sandbox_thread_data();
#  define RLBOX_NOOPSTATICINDIRECT_SANDBOX_STATIC_VARIABLES()                                \
    thread_local rlbox::rlbox_noopstaticindirect_sandbox_thread_data                         \
      rlbox_noopstaticindirect_sandbox_thread_info{ 0, 0 };                                  \
    namespace rlbox {                                                          \
      rlbox_noopstaticindirect_sandbox_thread_data* get_rlbox_noopstaticindirect_sandbox_thread_data()     \
      {                                                                        \
        return &rlbox_noopstaticindirect_sandbox_thread_info;                                \
      }                                                                        \
    }                                                                          \
...
public:
  ...
  // Thus transfer is a noopstaticindirect
...
private:
  ...
  thread_local static inline rlbox_noopstaticindirect_sandbox_thread_data thread_data{ 0, 0 };
#endif
  ...
  {
#ifdef RLBOX_EMBEDDER_PROVIDES_TLS_STATIC_VARIABLES
    auto& thread_data = *get_rlbox_noopstaticindirect_sandbox_thread_data();
#endif
...
protected:
  ...
  static inline void* impl_get_unsandboxed_pointer_no_ctx(
    ...
    rlbox_noopstaticindirect_sandbox* (*/* expensive_sandbox_finder */)(
      const void* example_unsandboxed_ptr))
  {
    return p;
  }
  ...
  {
    // There isn't any sandbox memory for the noopstaticindirect_sandbox as we just redirect
...
#define rlbox_noopstaticindirect_sandbox_lookup_symbol(func_name)                            \
  rlbox::hide_val_id(reinterpret_cast<void*>(&func_name)) /* NOLINT */
  ...
  static inline std::pair<rlbox_noopstaticindirect_sandbox*, void*>
