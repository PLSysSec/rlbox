...
public:
  ...
  /**
   * @brief Create a "fake" pointer referring to a location in the application memory
   *
  ...
  app_pointer<T*, T_Sbx> get_app_pointer(T* ptr) {
    ...
    // Right now we simply assume that any integer can be converted to a valid pointer in the sandbox
    // This may not be true for some sandboxing mechanism plugins in the future
    // In this case, we will have to come up with something more clever to construct indexes that look like valid pointers
    // Add a check for now to make sure things work fine
    detail::dynamic_check(
      is_pointer_in_sandbox_memory(idx_as_ptr),
      "App pointers are not currently supported for this rlbox sandbox plugin. Please file a bug.");
    auto ret = app_pointer<T*, T_Sbx>(&app_ptr_map, idx, reinterpret_cast<T*>(idx_as_ptr));
    return ret;
  }

  /**
   * @brief The mirror of get_app_pointer. Take a tainted pointer which is actually an app_pointer, and get the application location being pointed to
   *
