...
public:
    app_pointer_map() {
        // ensure we can't use app pointer 0 as this is sometimes confused as null by the sandbox
    ...
    void remove_app_ptr(T_PointerType idx) {
        ...
        detail::dynamic_check(it != pointer_map.end(), "Error: removing a non-existing app pointer");
    ...
    void* lookup_index(T_PointerType idx) {
        ...
        auto it = pointer_map.find(idx);
        detail::dynamic_check(it != pointer_map.end(), "Error: looking up a non-existing app pointer");
        return it->second;
    }
