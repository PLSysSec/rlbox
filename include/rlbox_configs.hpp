 ...
 * the static checks used by rlbox to runtime errors (abort()/exceptions). This
 * is meant to be used primarily by the tests to ensure easy testing.
 * - `RLBOX_DONT_OVERESTIMATE_CLASS_SIZES` - Define to disable the RLBox
 * approximations that allow calling @ref
 * rlbox::rlbox_sandbox::malloc_in_sandbox() to allocate a class without
